...
export const fetchNewsAPIArticles = async (query, filters) => {
  const searchUrl = `https://newsapi.org/v2/everything?q=language=en&${query}&from=${filters.date}`;
  const topHeadUrl = `https://newsapi.org/v2/top-headlines?language=en&country=ind&category=${filters.category}`;
  const categoryUrl = `https://newsapi.org/v2/top-headlines?language=en&country=ind&category=${filters.category}`
